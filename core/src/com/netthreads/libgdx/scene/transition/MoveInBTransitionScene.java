...
/**
 * Transition: Move in from bottom.
 */
public class MoveInBTransitionScene extends TransitionScene {
    private static Pool<MoveInBTransitionScene> _pool = new Pool<MoveInBTransitionScene>() {
        @Override
        protected MoveInBTransitionScene newObject() {
            MoveInBTransitionScene transitionScene = new MoveInBTransitionScene();
     ...
     * Create transition.
     *
     * @param inScene        The incoming scene.
     * @param outScene       The outgoing scene.
     * @param durationMillis The duration of transition.
     * @param easeEquation   The easing type.
     */
    public static MoveInBTransitionScene $(Scene inScene, Scene outScene, int durationMillis, TweenEquation easeEquation) {
        MoveInBTransitionScene transitionScene = _pool.obtain();
    ...
    public void enter() {
        ...
        Timeline inTimeline = Timeline.createSequence()
                .beginSequence()
                .push(Tween.to(getInSceneRoot(), GroupAccessor.POSITION_XY, 0).target(0, -getInScene().getHeight()).ease(getEaseEquation()))
        ...
        Timeline outTimeline = Timeline.createSequence()
                ...
                .push(Tween.to(getOutSceneRoot(), GroupAccessor.POSITION_XY, getDurationMillis()).target(0, getOutScene().getHeight()).ease(getEaseEquation()))
