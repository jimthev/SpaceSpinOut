...
/**
 * Transition: Move in from left.
 */
public class MoveInLTransitionScene extends TransitionScene {
    private static final int DEFAULT_INITIAL_POOL_SIZE = 4;

    private static Pool<MoveInLTransitionScene> _pool = new Pool<MoveInLTransitionScene>(DEFAULT_INITIAL_POOL_SIZE) {
        @Override
        protected MoveInLTransitionScene newObject() {
            MoveInLTransitionScene transitionScene = new MoveInLTransitionScene();
     ...
     */
    public static MoveInLTransitionScene $(Scene inScene, Scene outScene, int durationMillis, TweenEquation easeEquation) {
        MoveInLTransitionScene transitionScene = _pool.obtain();
    ...
    public void enter() {
        ...
        Timeline outTimeline = Timeline.createSequence()
                ...
                .push(Tween.to(getOutSceneRoot(), GroupAccessor.POSITION_XY, getDurationMillis()).target(getOutScene().getWidth(), 0).ease(getEaseEquation()))
